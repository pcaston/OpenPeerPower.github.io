...
Users can be managed in Open Peer Power by the owner. Go to the configuration panel and click on _Users_.
...
This provider can be used to integrate Open Peer Power with arbitrary external authentication services, from plaintext databases over LDAP to RADIUS. A compatible script for LDAP authentication is [this one](https://github.com/efficiosoft/ldap-auth-sh), for instance.
